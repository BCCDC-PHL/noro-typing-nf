process mask_low_coverage {
    ...
    tuple val(sample_id), path("${output_name}_bedtools_provenance.yml"), emit: provenance


    script:
    output_name = "${sample_id}_${task.ext.workflow}"
    """
    printf -- "- process_name: mask_low_coverage\\n" > ${output_name}_bedtools_provenance.yml
...
process make_consensus {
    ...
    tuple val(sample_id), path("${output_name}_bcftools_provenance.yml"), emit: provenance

    script:
    output_name = "${sample_id}_${task.ext.workflow}"
    """
    printf -- "- process_name: make_consensus\\n" > ${output_name}_bcftools_provenance.yml
